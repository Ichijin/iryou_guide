<div class="container" id="search">
	...
	<div class="search_caption"><h3>条件で検索する</h3><p>全国にある看護・医療系学校の<br class="sp">入試情報を検索できる！</p></div>
		<div class="contents_wrap">
			<form id="topForm" class="drop_menu_flex" method="POST" action="pages/search">
				<div class="drop_Menu">
					<p>学校区分</p>
				...
				<div class="drop_Menu">
					...
					<select class="select01" name="entrance_outline">
						...
						<option value="大学入学共通テスト利用入試あり（公募推薦）">大学入学共通テスト利用入試あり（公募推薦）</option>
				...
				<div class="drop_Menu">
					...
					<div class="select_date" name="entrance_term">
						<input type="date" name="term_from" id="term_from"></input><span>-</span><input type="date" name="term_end" id="term_end"></input>
					</div>
				</div>
				<div class="drop_Menu">
					<p>入試科目</p>
					<select class="select01" name="entrance_subject">
						<option value="">すべて</option>
						...
						<option value="学科試験（その他）">学科試験（その他）</option>
				...
				<button class="SearchButton" type="submit" id="btn-submit" name="btn-submit" onclick="spinerEvent();return false;">検索&nbsp;<img src="https://www.ishin.jp/iryou_guide/images/icon_search.svg" alt="search" width="15" height="15"></button><br>
            </form>
		</div>


<script>
	function spinerEvent() {
		var err = 0;
		if($('#term_from').val() != '' && $('#term_end').val() != ''){
			if($('#term_from').val() > $('#term_end').val()){
				alert('出願期間の指定が不正です。');
				err = 1;
			}
